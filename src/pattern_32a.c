...
static inline uint32_t peek_counter_(struct embc_pattern_32a_tx_s * self) {
    return (((uint32_t) ~self->counter) << 16) | self->counter;
}

static inline void advance_counter_(struct embc_pattern_32a_tx_s * self) {
    ++self->counter;
}

static inline uint32_t peek_shift_(struct embc_pattern_32a_tx_s * self) {
    return self->shift32;
}

static inline void advance_shift_(struct embc_pattern_32a_tx_s * self) {
    self->shift32 = (0 == self->shift32) ? 1 : (self->shift32 << 1);
}

static inline uint32_t peek_(struct embc_pattern_32a_tx_s * self) {
    uint32_t value;
...
static inline void advance_(struct embc_pattern_32a_tx_s * self) {
    if (TOGGLE_SHIFT == self->toggle) {
        advance_shift_(self);
...
uint32_t embc_pattern_32a_tx_next(struct embc_pattern_32a_tx_s * self) {
    uint32_t value;
    if (TOGGLE_SHIFT == self->toggle) {
        value = peek_shift_(self);
        advance_shift_(self);
 ...
 */
static bool resync(struct embc_pattern_32a_rx_s * self, uint32_t v1, uint32_t v2) {
    struct embc_pattern_32a_tx_s tx_old = self->tx;
    ...
    if (tx_old.toggle != self->tx.toggle) {
        embc_pattern_32a_tx_next(&tx_old);
        incr = 1;
    }
    ...
    if (delta > (EMBC_PATTERN_32A_PERIOD / 2)) {
        // presume duplicated
        self->duplicate_count = EMBC_PATTERN_32A_PERIOD - delta - incr;
    } else {
        self->missing_count += incr + delta;
    }

    // Advance to next expected
    embc_pattern_32a_tx_next(&self->tx);
    embc_pattern_32a_tx_next(&self->tx);
