...
static bool validate_crc(uint8_t const * frame) {
    uint16_t frame_sz;
    if (parse_frame_type(frame) == EMBC_FRAMER_FT_DATA) {
        frame_sz = parse_data_payload_length(frame) + EMBC_FRAMER_OVERHEAD_SIZE;
    } else {
...
static void handle_framing_error(struct embc_framer_s * self) {
    self->state = ST_SOF1;
    if (self->is_sync) {
        ++self->status.resync;
        self->is_sync = 0;
...
static void handle_framing_error_discard(struct embc_framer_s * self) {
...
static void reprocess_buffer(struct embc_framer_s * self) {
    handle_framing_error(self);
        ...
        switch (self->state) {
            ...
            case ST_SOF2:
                ...
                if (self->buf[1] == EMBC_FRAMER_SOF2) {
                    ...
                    EMBC_LOGW("Expected SOF2 got 0x%02x", self->buf[1]);
                    handle_framing_error_discard(self);
                }
                break;
            ...
            case ST_FRAME_TYPE:
                if (self->buf[2] & 0x18) {
                    EMBC_LOGW("Invalid reserved bits: 0x%02x", self->buf[2]);
                    handle_framing_error_discard(self);
                    break;
                }
                switch (parse_frame_type(self->buf)) {
                    case EMBC_FRAMER_FT_DATA:
                        ...
                        break;
                    case EMBC_FRAMER_FT_ACK_ALL:            /* Intentional fall-through */
                    case EMBC_FRAMER_FT_ACK_ONE:            /* Intentional fall-through */
                    case EMBC_FRAMER_FT_NACK_FRAME_ID:      /* Intentional fall-through */
                    case EMBC_FRAMER_FT_NACK_FRAMING_ERROR: /* Intentional fall-through */
                    case EMBC_FRAMER_FT_RESET:
                        self->state = ST_STORE;
                        self->length = EMBC_FRAMER_LINK_SIZE;
                        break;
                    default:
                        handle_framing_error_discard(self);
                        break;
            ...
            case ST_DATA_LENGTH:
                self->state = ST_STORE;
                self->length = parse_data_payload_length(self->buf) + EMBC_FRAMER_OVERHEAD_SIZE;
                break;
...
int32_t embc_framer_construct_data(uint8_t * b, uint16_t frame_id, uint32_t metadata,
    ...
    b[EMBC_FRAMER_HEADER_SIZE + msg_size + 2] = (crc >> 16) & 0xff;
    b[EMBC_FRAMER_HEADER_SIZE + msg_size + 3] = (crc >> 24) & 0xff;
    return 0;
}
...
int32_t embc_framer_construct_link(uint8_t * b, enum embc_framer_type_e frame_type, uint16_t frame_id) {
    ...
    uint32_t crc = embc_crc32(0, b + 2, 2);
    b[4] = crc & 0xff;
    b[5] = (crc >> 8) & 0xff;
    b[6] = (crc >> 16) & 0xff;
    b[7] = (crc >> 24) & 0xff;
