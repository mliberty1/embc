...
struct embc_pool_element_s {
    ...
    struct embc_pool_s * pool;
    union {
        /** The status when allocated. */
        struct embc_pool_alloc_s alloc;
...
EMBC_STATIC_ASSERT((sizeof(intptr_t) != 4) ||
                           (8 == sizeof(struct embc_pool_element_s)),
                   embc_pool_element_header_size_32);
EMBC_STATIC_ASSERT((sizeof(intptr_t) != 8) ||
                           (16 == sizeof(struct embc_pool_element_s)),
                   embc_pool_element_header_size_64);
...
struct embc_pool_s {
    ...
    int32_t obj_size;
    embc_pool_constructor constructor;
    embc_pool_destructor destructor;
};
...
static struct embc_pool_size_s embc_pool_size(int32_t obj_count, int32_t obj_size) {
    struct embc_pool_size_s sz;
    sz.obj_hdr = EMBC_ROUND_UP_TO_MULTIPLE(obj_size, EMBC_POOL_ALIGNMENT);
    sz.obj_data = EMBC_ROUND_UP_TO_MULTIPLE(sizeof(struct embc_pool_element_s), EMBC_POOL_ALIGNMENT);
...
int32_t embc_pool_instance_size(int32_t obj_count, int32_t obj_size) {
    struct embc_pool_size_s sz = embc_pool_size(obj_count, obj_size);
    return sz.sz;
}

int32_t embc_pool_initialize(
        struct embc_pool_s * self, int32_t obj_count, int32_t obj_size,
        embc_pool_constructor constructor, embc_pool_destructor destructor) {
...
void * embc_pool_alloc(struct embc_pool_s * self) {
    struct embc_pool_element_s * next = self->free_head;
    EMBC_ASSERT(next);
...
static inline struct embc_pool_element_s * get_obj_header(void * obj) {
    uint8_t * m = (uint8_t *) obj;
    m -= sizeof(struct embc_pool_element_s);
    struct embc_pool_element_s * hdr = (struct embc_pool_element_s *) m;
    EMBC_ASSERT(hdr->d.alloc.status == STATUS_ALLOC);  // otherwise already free!
    return hdr;
}

void embc_pool_incr(void * obj) {
    struct embc_pool_element_s * hdr = get_obj_header(obj);
    EMBC_ASSERT(hdr->d.alloc.count < 65535);
    ++hdr->d.alloc.count;
}

bool embc_pool_decr(void * obj) {
    struct embc_pool_element_s * hdr = get_obj_header(obj);
