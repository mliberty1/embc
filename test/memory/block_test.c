...
static int setup1(void ** state) {
    ...
    t->s = test_calloc(1, embc_mblock_instance_size(256, 8));
    assert_non_null(t->s);
    assert_int_equal(0, embc_mblock_initialize(t->s, t->memory, 256, 8));
    *state = t;
    return 0;
}
...
static void alloc_free_alloc_free(void **state) {
    struct test_s * t = (struct test_s *) *state;
    void * p1 = embc_mblock_alloc(t->s, 1);
    embc_mblock_free(t->s, p1, 1);
    void * p2 = embc_mblock_alloc(t->s, 1);
    embc_mblock_free(t->s, p2, 1);
    assert_ptr_equal(p1, p2);
}

static void alloc_until_full(void **state) {
    struct test_s * t = (struct test_s *) *state;
    uint8_t * p1 = (uint8_t *) embc_mblock_alloc(t->s, 1);
    uint8_t * p2 = (uint8_t *) embc_mblock_alloc(t->s, 1);
    assert_ptr_equal(p1 + 8, p2);
    (uint8_t *) embc_mblock_alloc(t->s, 8 * 30); // fill
    expect_assert_failure(embc_mblock_alloc(t->s, 1));
}

static void alloc_until_full_unsafe(void **state) {
    struct test_s * t = (struct test_s *) *state;
    uint8_t * p1 = (uint8_t *) embc_mblock_alloc(t->s, 128);
    assert_non_null(p1);
    uint8_t * p2 = (uint8_t *) embc_mblock_alloc(t->s, 64);
    assert_non_null(p2);
    assert_null(embc_mblock_alloc_unsafe(t->s, 128));
    uint8_t * p3 = (uint8_t *) embc_mblock_alloc_unsafe(t->s, 64);
    assert_non_null(p3);
}
...
static void alloc_free_middle_alloc_smaller(void **state) {
    struct test_s * t = (struct test_s *) *state;
    uint8_t * p[256/32];
    for (int i = 0; i < (int) EMBC_ARRAY_SIZE(p); ++i) {
        p[i] = embc_mblock_alloc(t->s, 32);
    }
    embc_mblock_free(t->s, p[1], 32);
    embc_mblock_free(t->s, p[3], 32);
    embc_mblock_alloc(t->s, 16);
    embc_mblock_alloc(t->s, 16);
    embc_mblock_alloc(t->s, 16);
    embc_mblock_alloc(t->s, 16);
}
...
static void alloc_fragmentation(void **state) {
    ...
    embc_mblock_free(t->s, p[1], 32);
    embc_mblock_free(t->s, p[3], 32);
    expect_assert_failure(embc_mblock_alloc(t->s, 64));
}
