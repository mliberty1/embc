...
static void init_alloc_free_one(void **state) {
    (void) state;
    embc_buffer_initialize(SIZES1, EMBC_ARRAY_SIZE(SIZES1));
    struct embc_buffer_s * b = embc_buffer_alloc(10);
    assert_non_null(b);
    assert_int_equal(32, embc_buffer_capacity(b));
    embc_buffer_free(b);
    embc_buffer_finalize();
}
...
static void init_alloc_until_empty(void **state) {
    ...
    for (int i = 0; i < 8; ++i) {
        b[i] = embc_buffer_alloc(30);
    }
    assert_ptr_not_equal(b[0], b[1]);
    expect_assert_failure(embc_buffer_alloc(30));
    for (int i = 0; i < 8; ++i) {
        embc_buffer_free(b[i]);
        assert_ptr_equal(b[i], embc_buffer_alloc(30));
    }
    embc_buffer_finalize();
}
...
static void buffer_write_str(void **state) {
    (void) state;
    embc_buffer_initialize(SIZES1, EMBC_ARRAY_SIZE(SIZES1));
    struct embc_buffer_s * b = embc_buffer_alloc(30);
    assert_int_equal(32, embc_buffer_capacity(b));
