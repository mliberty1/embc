...
enum {
    ...
    ST_B,
    ST_C,
    ST_NULL = EMBC_STATE_NULL,
    ST_ANY = EMBC_STATE_ANY,
    ST_SKIP = EMBC_STATE_SKIP,
};
...
enum {
    ...
    EV_B,
    EV_C,
    EV_ANY = EMBC_EVENT_ANY,
    EV_RESET = EMBC_EVENT_RESET,
    EV_ENTER = EMBC_EVENT_ENTER,
    EV_EXIT = EMBC_EVENT_EXIT,
};
...
struct fsm_test_state_s {
    struct embc_fsm_s fsm;
    struct transition_log_s log[128];
    int log_entries;
};

static void transition_log(struct fsm_test_state_s * self, embc_fsm_event_t event) {
    self->log[self->log_entries].state = self->fsm.state;
    self->log[self->log_entries].event = event;
    ++self->log_entries;
}

#define H(x) (embc_fsm_handler) x
#define H_DEF(fn_) \
    static embc_fsm_state_t fn_(struct fsm_test_state_s * self, \
                              embc_fsm_event_t event)
...
H_DEF(signal_ev_c) {
    (void) event;
    embc_fsm_event(&self->fsm, EV_C);
    return ST_ANY;
}

static const struct embc_fsm_state_s my_states[] = {
...
static const struct embc_fsm_transition_s my_transitions[] = {
...
static const struct embc_fsm_transition_s my_transitions_skip[] = {
// current, next, event, handler
...
static const struct embc_fsm_transition_s my_transitions_null[] = {
// current, next, event, handler
...
static int setup(void ** state) {
    ...
    s->fsm.name = "my_fsm";
    s->fsm.states_count = EMBC_ARRAY_SIZE(my_states);
    s->fsm.transitions = my_transitions;
    s->fsm.transitions_count = EMBC_ARRAY_SIZE(my_transitions);
...
static void empty(void **state) {
    ...
    expect_assert_failure(embc_fsm_initialize(&s->fsm));
}
...
static void transition_skip(void **state) {
    ...
    s->fsm.transitions_count = EMBC_ARRAY_SIZE(my_transitions_skip);
    embc_fsm_initialize(&s->fsm);
    s->log_entries = 0;
    embc_fsm_event(&s->fsm, EV_A);
...
static void transition_null(void **state) {
    ...
    s->fsm.transitions_count = EMBC_ARRAY_SIZE(my_transitions_null);
    embc_fsm_initialize(&s->fsm);
    s->log_entries = 0;
    embc_fsm_event(&s->fsm, EV_A);
...
static const struct embc_fsm_transition_s transitions_reentrant[] = {
// current, next, event, handler
...
static void reentrant(void **state) {
    ...
    s->fsm.transitions_count = EMBC_ARRAY_SIZE(transitions_reentrant);
    embc_fsm_initialize(&s->fsm);
    embc_fsm_event(&s->fsm, EV_A);
