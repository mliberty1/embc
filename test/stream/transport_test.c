...
static int setup(void ** state) {
    ...
    embc_transport_on_event_cbk(self->t, EMBC_DL_EV_TX_CONNECTED);
...
static void test_recv(void ** state) {
    ...
    embc_transport_on_recv_cbk(self->t, 0xABCD81, DATA1, sizeof(DATA1));

    expect_recv(1, EMBC_TRANSPORT_SEQ_MIDDLE, 0, DATA1, sizeof(DATA1));
    embc_transport_on_recv_cbk(self->t, 0x01, DATA1, sizeof(DATA1));

    expect_recv(1, EMBC_TRANSPORT_SEQ_STOP, 0, DATA1, sizeof(DATA1));
    embc_transport_on_recv_cbk(self->t, 0x41, DATA1, sizeof(DATA1));
