...
void on_data(void * user_data, uint16_t frame_id,
...
static void expect_data(uint16_t frame_id,
                        uint8_t port_id, uint16_t message_id,
                        uint8_t const *msg_buffer, uint32_t msg_size) {
    expect_value(on_data, frame_id, frame_id);
    expect_value(on_data, port_id, port_id);
    expect_value(on_data, message_id, message_id);
    expect_value(on_data, msg_size, msg_size);
    expect_memory(on_data, msg_buffer, msg_buffer, msg_size);
}

static void send_data(struct test_s * self, uint16_t frame_id,
                      uint8_t port_id, uint16_t message_id,
                      uint8_t const *msg_buffer, uint32_t msg_size) {
    uint8_t b[EMBC_FRAMER_MAX_SIZE];
    assert_int_equal(0, embc_framer_construct_data(b, frame_id, port_id, message_id, msg_buffer, msg_size));
    expect_data(frame_id, port_id, message_id, msg_buffer, msg_size);
    embc_framer_ll_recv(&self->f, b, msg_size + EMBC_FRAMER_OVERHEAD_SIZE);
}

void on_link(void * user_data, enum embc_framer_type_e frame_type, uint16_t frame_id) {
    struct test_s * self = (struct test_s *) user_data;
    (void) self;
...
void on_framing_error(void * user_data) {
    struct test_s * self = (struct test_s *) user_data;
    (void) self;
...
static int setup(void ** state) {
    ...
    embc_framer_construct_data(self->frame1, 1, 2, 3, PAYLOAD1, sizeof(PAYLOAD1));
...
static void test_ack_all(void ** state) {
    ...
    send_link(self, EMBC_FRAMER_FT_ACK_ALL, EMBC_FRAMER_PORTS_MAX);
}
...
static void test_nack_frame_id(void ** state) {
    ...
    send_link(self, EMBC_FRAMER_FT_NACK_FRAME_ID, EMBC_FRAMER_PORTS_MAX);
}
...
static void test_data(void ** state) {
    struct test_s *self = (struct test_s *) *state;
    send_data(self, 1, 2, 3, PAYLOAD1, sizeof(PAYLOAD1));
}
...
static void test_data_split(void ** state) {
    ...
    for (uint16_t frame_id = 1; frame_id < (sz - 1); ++frame_id) {
        assert_int_equal(0, embc_framer_construct_data(b, frame_id, port_id, message_id, PAYLOAD1, sizeof(PAYLOAD1)));
        expect_data(frame_id, port_id, message_id, PAYLOAD1, sizeof(PAYLOAD1));
...
static void test_data_min_length(void ** state) {
    struct test_s *self = (struct test_s *) *state;
    uint8_t b[1] = {0x11};
    send_data(self, 1, 2, 3, b, sizeof(b));
}
...
static void test_construct_data_checks(void ** state) {
    ...
    (void) self;
    uint8_t b[EMBC_FRAMER_MAX_SIZE];
    assert_int_equal(EMBC_ERROR_PARAMETER_INVALID, embc_framer_construct_data(b, EMBC_FRAMER_FRAME_ID_MAX + 1, 0, 0, PAYLOAD1, sizeof(PAYLOAD1)));
    assert_int_equal(EMBC_ERROR_PARAMETER_INVALID, embc_framer_construct_data(b, 0, EMBC_FRAMER_PORTS_MAX + 1, 0, PAYLOAD1, sizeof(PAYLOAD1)));
    assert_int_equal(EMBC_ERROR_PARAMETER_INVALID, embc_framer_construct_data(b, 0, 0, 0, PAYLOAD1, 0));
    assert_int_equal(EMBC_ERROR_PARAMETER_INVALID, embc_framer_construct_data(b, 0, 0, 0, PAYLOAD1, EMBC_FRAMER_PAYLOAD_MAX_SIZE + 1));
}
...
int main(void) {
    ...
    const struct CMUnitTest tests[] = {
            ...
            //cmocka_unit_test_setup_teardown(test_one_many_sof_split, setup, teardown),
            //cmocka_unit_test_setup_teardown(test_one_initial_garbage, setup, teardown),
            //cmocka_unit_test_setup_teardown(test_three, setup, teardown),
