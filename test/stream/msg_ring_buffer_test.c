...
static int setup(void ** state) {
    ...
    embc_mrb_init(&self->mrb, self->b, SZ);
    *state = self;
    return 0;
}
...
static void test_alloc_until_full(void ** state) {
    ...
    uint32_t sz = 0;
    assert_ptr_equal(self->b + 4, embc_mrb_pop(&self->mrb, &sz));
...
static void test_alloc_sizes(void ** state) {
    ...
    for (uint32_t sz = 1; sz < ((SZ / 2) - 6); ++sz) {
        for (int i = 0; i < 32; ++i) {
            ...
            assert_non_null(b);
            b[0] = i;
            b = embc_mrb_pop(&self->mrb, &sz_out);
            assert_non_null(b);
...
static void test_alloc_sizes_keep_not_empty(void ** state) {
    ...
    embc_mrb_init(&self->mrb, self->b, sizeof(self->b));
...
static void test_clear(void ** state) {
    struct test_s *self = (struct test_s *) *state;
    uint32_t sz = 1;
    assert_non_null(embc_mrb_alloc(&self->mrb, 8));
    embc_mrb_clear(&self->mrb);
    assert_null(embc_mrb_pop(&self->mrb, &sz));
