...
struct embc_pattern_32a_tx_s tx1;
struct embc_pattern_32a_tx_s tx2;
struct embc_pattern_32a_rx_s rx1;
...
static int setup(void ** state) {
    (void) state;
    embc_pattern_32a_tx_initialize(&tx1);
    embc_pattern_32a_tx_initialize(&tx2);
    embc_pattern_32a_rx_initialize(&rx1);
    return 0;
}
...
static void skip_case(int offset, int before, int skip, int after) {
    ...
    for (int i = 0; i < before; ++i) {
        value = embc_pattern_32a_tx_next(&tx1);
        embc_pattern_32a_rx_next(&rx1, value);
    }
